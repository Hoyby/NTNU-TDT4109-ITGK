...
import math
def round_up(n, desimal=0):
    multiplier = 10 ** desimal
    return math.ceil(n * multiplier) / multiplier
    
tall = float(input("Gi inn et desimaltall: "))
desimal = int(input("Antall desimaler i avrunding:"))
print("Avrundet til 0 desimaler: ", round_up(tall, 2))
...
heltall = (input("Oppgi heltallsdelen av tallet (det foran punktum): "))
desimal = (input("Oppgi desimaldelen av tallet (det bak punktum): "))
antall_desimaler = (input("Oppgi ønsket antall desimaler i avrunding: "))


tall = int(heltall+desimal)
kutt = max(0, len(desimal) - int(antall_desimaler))
done = str((int(tall / 10 ** int(kutt) + 0.5))/10**int(antall_desimaler))

print(f"{heltall},{desimal}", "avrundet til", f"{antall_desimaler}", "desimaler blir", f"{done}")
...
name = str(input("Jeg heter: "))
print("The name is", name.split()[1], "," , name)
