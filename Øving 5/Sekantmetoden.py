...
def secant_method(x0, x1, func, tol):
    ...
    diff = abs(x1 - x0)
    while diff < tol:
        x2 = x1 - func(x1)/differentiate(x1, x0, func)
        diff = abs(x2 - x1)
        x0, x1 = x1, x2
    return x2


print("Funksjonen nærmer seg et nullpunkt når x = %s, da er f(x) =" % (secant_method(11, 13, f, 0.00001)))
print("Funksjonen nærmer seg et nullpunkt når x = %s, da er f(x) =" % (secant_method(1, 2, g, 0.00001)))
print("Funksjonen nærmer seg et nullpunkt når x = %s, da er f(x) =" % (secant_method(0, 1, g, 0.00001)))
