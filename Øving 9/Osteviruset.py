...
def infected_cheese(infected_shelves):
    infected = []
    for keys in cheeses:
        for i in range(0, len(cheeses[keys])):
            shelve = cheeses[keys][i]
            if shelve[:-(len(shelve)-shelve.find('-'))] in infected_shelves:
                infected += [keys]
    return (list(set(infected)))


infected_cheese(["B13", "B14", "B15", "A234", "A235", "C31"])
...
def not_infected_cheese(infected_shelves):
    for keys in cheeses:
        if keys not in infected_cheese(infected_shelves):
            for i in cheeses[keys]:
                print("{} {:2} {} {}".format(i, "", "\t", keys))
        infected_cheese(infected_shelves)
        
not_infected_cheese(["B13", "B14", "B15", "A234", "A235", "C31"])
